...
std::vector<DrawingAPI_Shape_Quad*> DRAWINGS_QUAD;
...
namespace DrawingAPILuaAPI {
	...
	namespace Lua {
		...
		*/
		...
		namespace Quad {
			...
			static int __gc(lua_State* LS) {
				luaL_checktype(LS, 1, LUA_TUSERDATA);
				DrawingAPI_Shape_Quad& Object = *static_cast<DrawingAPI_Shape_Quad*>(lua_touserdata(LS, 1));
				for (size_t Iteration = 0; Iteration < DRAWINGS_QUAD.size(); Iteration++)
				{
					if (DRAWINGS_QUAD[Iteration] == &Object) {
						DRAWINGS_QUAD.erase(DRAWINGS_QUAD.begin() + Iteration);
					}
				}
				return 0;
			}
			...
			static int __index(lua_State* LS) {
				...
				if (!std::strcmp(Key, "Remove")) {
					...
					lua_getglobal(LS, "Vector2");
					lua_getfield(LS, -1, "new");
					lua_pushnumber(LS, Object.PointA.Position_X);
					lua_pushnumber(LS, Object.PointA.Position_Y);
					lua_call(LS, 2, 1);
					return 1;
				}
				else if (!std::strcmp(Key, "PointB")) {
					lua_getglobal(LS, "Vector2");
					lua_getfield(LS, -1, "new");
					lua_pushnumber(LS, Object.PointB.Position_X);
					lua_pushnumber(LS, Object.PointB.Position_Y);
					lua_call(LS, 2, 1);
					return 1;
				}
				else if (!std::strcmp(Key, "PointC")) {
					lua_getglobal(LS, "Vector2");
					lua_getfield(LS, -1, "new");
					lua_pushnumber(LS, Object.PointC.Position_X);
					lua_pushnumber(LS, Object.PointC.Position_Y);
					lua_call(LS, 2, 1);
					return 1;
				}
				else if (!std::strcmp(Key, "PointD")) {
					lua_getglobal(LS, "Vector2");
					lua_getfield(LS, -1, "new");
					lua_pushnumber(LS, Object.PointD.Position_X);
					lua_pushnumber(LS, Object.PointD.Position_Y);
					lua_call(LS, 2, 1);
					return 1;
				}
				else if (!std::strcmp(Key, "Size")) {
			...
			static int __newindex(lua_State* LS) {
				...
				if (!std::strcmp(Key, "Visible")) {
					...
					lua_pushvalue(LS, 3);
					lua_getfield(LS, -1, "X");
					Object.PointA.Position_X = luaL_checknumber(LS, -1);
					lua_getfield(LS, -2, "Y");
					Object.PointA.Position_Y = luaL_checknumber(LS, -1);
					return 0;
				}
				else if (!std::strcmp(Key, "PointB")) {
					luaL_checktype(LS, 3, LUA_TUSERDATA);
					lua_pushvalue(LS, 3);
					lua_getfield(LS, -1, "X");
					Object.PointB.Position_X = luaL_checknumber(LS, -1);
					lua_getfield(LS, -2, "Y");
					Object.PointB.Position_Y = luaL_checknumber(LS, -1);
					return 0;
				}
				else if (!std::strcmp(Key, "PointC")) {
					luaL_checktype(LS, 3, LUA_TUSERDATA);
					lua_pushvalue(LS, 3);
					lua_getfield(LS, -1, "X");
					Object.PointC.Position_X = luaL_checknumber(LS, -1);
					lua_getfield(LS, -2, "Y");
					Object.PointC.Position_Y = luaL_checknumber(LS, -1);
					return 0;
				}
				else if (!std::strcmp(Key, "PointD")) {
					luaL_checktype(LS, 3, LUA_TUSERDATA);
					lua_pushvalue(LS, 3);
					lua_getfield(LS, -1, "X");
					Object.PointD.Position_X = luaL_checknumber(LS, -1);
					lua_getfield(LS, -2, "Y");
					Object.PointD.Position_Y = luaL_checknumber(LS, -1);
					return 0;
				}
				else if (!std::strcmp(Key, "Size")) {
					luaL_checktype(LS, 3, LUA_TUSERDATA);
	...
	static int New(lua_State* LS) {
		...
		else if (!std::strcmp(Object, "Quad")) {
			ObjectUD = lua_newuserdata(LS, sizeof(DrawingAPI_Shape_Quad));
			lua_getfield(LS, LUA_REGISTRYINDEX, "DrawingObject_Quad");
			lua_setmetatable(LS, -2);
			new (ObjectUD)DrawingAPI_Shape_Quad;
			DRAWINGS_QUAD.push_back((DrawingAPI_Shape_Quad*)ObjectUD);
			return 1;
		}
		else {
			luaL_error(LS, "Invalid DrawingObject!");
			return 0;
		}
		return 0;
	}
	...
	void InitiateLuaAPI(lua_State* LS) {
		...
		luaL_newmetatable(LS, "DrawingObject_Quad");
		lua_pushcfunction(LS, Lua::Quad::__gc);
		lua_setfield(LS, -2, "__gc");
		lua_pushcfunction(LS, Lua::Quad::__index);
		lua_setfield(LS, -2, "__index");
		lua_pushcfunction(LS, Lua::Quad::__newindex);
		lua_setfield(LS, -2, "__newindex");
		lua_pop(LS, 1);
...
unsigned __int64 _startings_tick;
unsigned __int64 _finishs_tick;
...
namespace Connection1 {
	...
	void Start() {
		Reset();
		_startings_tick = GetTickCount64();
	}

	double GetElapsedTime() {
		if (_startings_tick & _finishs_tick)
			return (_finishs_tick - _startings_tick) / 1000.0;
		return 0.0;
	}

	double Stop() {
		_finishs_tick = GetTickCount64();
...
bool DrawingAPI::InitiateDrawingAPI(lua_State* LS) {
	...
	*reinterpret_cast<void**>(VirtualTable + 32) = &DrawingPresent;
	...
	std::cout << ipurple << "[" << iyellow << "Initiated DrawingAPI.." << igreen << Connection1::Stop() << "ms" << ipurple << "]\n";
